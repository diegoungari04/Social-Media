...
3. **Estúdio** — escolha entre **6 variações de template**, edite texto, cores, fonte, foto e posição, e **exporte em PNG 1080×1350**. Copie a legenda pronta.
4. **Biblioteca** — salva seus posts (no navegador) para reabrir e reeditar.
...
css/app.css
js/store.js         # estado + localStorage (manual, config, biblioteca)
js/prompts.js       # prompts (inteligência de marca → posts)
js/ai.js            # cliente Claude API + fallback offline + parser JSON
js/templates.js     # 6 templates visuais (render do card 1080×1350)
js/studio.js        # edição, variações, zoom, export PNG
